...
module.exports = { //characters based on a value scale + averaging
	setInstance: function(instance){
		AsciiArt = instance;
	},
	render : function(image, callback){
		try{
			var width = image.options.width;
			var height = image.options.height;
			var distortion = 0.5;
			height = Math.floor(image.options.height*distortion);
			var newImage = new AsciiArt.Image.Image();
			newImage.src = image.canvas.toBuffer();
			var canvas = new AsciiArt.Image.Canvas(width, height);
